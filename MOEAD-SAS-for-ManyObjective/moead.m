...
%   selportion: the selection portion for the dynamic resource allocation
    ...
    global subproblems params itrCounter evalCounter rnduni EP numrun xxx Lev;
...
%     save(sprintf('data/%s/LEV_%d.mat',mop.name,numrun),'levs');
%     save(sprintf('data/%s/sub_xxx%d.mat',mop.name,numrun),'dxxx');
    %%%%%%
end

% The evoluation setp in MOEA/D
function evolve(mop)
	global subproblems idealpoint params rnduni EP selectionSize Lev ntimes;
...
%     end
    ...
    pop = [EP ind];
    L=min(Lev+ntimes*params.niche, length(pop));
    sorting(pop, L);
end
