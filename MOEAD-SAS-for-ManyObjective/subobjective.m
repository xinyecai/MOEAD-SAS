...
function [obj,d2] = bi(weight, ind, idealpoint)
    si = 10;
    mweight=sqrt(sum(weight.^2));
    tmp1=abs(sum((ind-repmat(idealpoint,1,size(ind,2))).*weight,1));
    tmp2=sqrt(sum(weight.^2,1));
    d1=tmp1./tmp2;
    tmp3=ind-repmat(idealpoint,1,size(ind,2))-repmat(d1./mweight,size(weight,1),1).*weight;
    d2=sqrt(sum(tmp3.^2,1));
    obj=d1+si*d2;
end
